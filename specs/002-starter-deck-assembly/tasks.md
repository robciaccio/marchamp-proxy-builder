...
- [ ] T048d [US1] Implement the decision route in `src/marchamp/api/routes.py`, the `decklist_candidate` field on the run, and the hold in `awaiting_cards` while it is undecided. The `decklist_printed` / `decklist_source_url` **report** fields belong to `assembly/report.py`, which US2 owns — T059 already tests every section of data-model.md § Assembly Report, the Decklist row included, so this task must not create a second report module ahead of it (FR-013b, FR-013d, FR-013e)
...
- [ ] T060 [US2] Implement `src/marchamp/assembly/report.py`, including the `decklist_printed` / `decklist_source_url` fields the US1 decklist work (T048a–T048d) prints but deliberately does not report — the report is US2's module, and until this task lands a Phase 3 pack prints its decklist card without saying so (SC-006j)
