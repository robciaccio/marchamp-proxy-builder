...
def test_valid_catalog_passes(catalog_dict, image_store):
    report = validate(load_catalog(catalog_dict), image_store)
    assert report.valid, report.errors


def test_all_errors_reported_together_not_first_and_stop(catalog_dict, image_store):
    ...
    report = validate(load_catalog(broken), image_store)
    kinds = {e.kind for e in report.errors}
    assert {"unknown_card_reference", "invalid_quantity", "missing_image_file"} <= kinds
    assert len(report.errors) >= 3


def test_preferred_printing_must_belong_to_that_card(catalog_dict, image_store):
    broken = copy.deepcopy(catalog_dict)
...
def test_missing_art_is_not_an_error_when_another_printing_exists(catalog_dict, image_store):
...
def test_duplicate_image_mapping_is_a_warning_not_an_error(catalog_dict, image_store):
    dup = copy.deepcopy(catalog_dict)
    dup["cards"][2]["printings"][0]["image"] = dup["cards"][1]["printings"][0]["image"]
    report = validate(load_catalog(dup), image_store)
    assert report.valid
    assert any(w.kind == "shared_image_file" for w in report.warnings)


def test_unreferenced_files_are_not_a_fault(catalog_dict, image_dir, image_store):
    (image_dir / "stray.tiff").write_bytes(b"II*\x00")
    assert validate(load_catalog(catalog_dict), image_store).valid


def test_double_sided_card_must_have_a_back(catalog_dict, image_store):
    broken = copy.deepcopy(catalog_dict)
...
def test_unsafe_image_path_is_rejected(catalog_dict, image_store):
    broken = copy.deepcopy(catalog_dict)
...
def test_preferred_printing_wins_when_present(catalog_dict, image_store):
    cat = load_catalog(catalog_dict)
    entry = cat.deck("testman-deck").entries[1]
    out = resolve_entry(cat, entry, image_store)
    assert out.outcome is ResolutionOutcome.PREFERRED
    assert out.printing.id == "sig1@pack"
    assert out.substitution is None


def test_stand_in_used_and_reported_when_preferred_art_is_absent(catalog_dict, image_store):
    cat = load_catalog(catalog_dict)
...
def test_stand_in_choice_is_deterministic(catalog_dict, image_store):
    ...
    entry = next(e for e in cat.deck("testman-deck").entries if e.card_id == "energy")
    picks = {resolve_entry(cat, entry, image_store).printing.id for _ in range(10)}
    assert picks == {"energy@aaa"}  # lowest id wins, never directory or hash order


def test_no_usable_printing_fails_rather_than_falling_back(catalog_dict, image_store):
