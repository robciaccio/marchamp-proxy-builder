...
def test_quantities_expand_to_one_face_per_copy(catalog_dict, image_store):
    # sig1 has quantity 2.
    cat = load_catalog(catalog_dict)
    faces = [
        f for p in paginate(cat, "testman-deck", PageSize.LETTER, image_store) for f in p.faces
    ]
    assert sum(1 for f in faces if f.card_id == "sig1") == 2


def test_double_sided_card_yields_two_faces(catalog_dict, image_store):
    cat = load_catalog(catalog_dict)
    faces = [
        f for p in paginate(cat, "testman-deck", PageSize.LETTER, image_store) for f in p.faces
    ]
...
def test_double_sided_faces_are_adjacent(catalog_dict, image_store):
...
def test_last_page_is_partially_filled_with_no_placeholders(catalog_dict, image_store):
    cat = load_catalog(catalog_dict)
    pages = paginate(cat, "testman-deck", PageSize.LETTER, image_store)
    assert len(pages) == 1
